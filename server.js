...
import { messages, conversations } from "./shared/schemas.js"; 
...
io.on("connection", (socket) => {
      ...
      if (conversation) {
        try {
          ...
          const receiverSocket = Array.from(io.sockets.sockets.values()).find(
            (s) => s.userId === data.receiverId.toString()
          );
  ...
  socket.on("markAsRead", async (conversationId) => {
    ...
    try {
      ...
      // Mesajları okuduğunu broadcast et
      ...
      socket.emit("error", "Mesajlar okundu işaretlenemedi");
    }
  });
